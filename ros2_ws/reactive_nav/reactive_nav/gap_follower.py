...
class ReactiveNavigator(Node):
    ...
    def _control_tick(self):
        ...
        # Compute sector mins
        left_min = self._sector_min(scan, center_angle=+math.pi / 2, half_width=self.side_angle)
        right_min = self._sector_min(scan, center_angle=-math.pi / 2, half_width=self.side_angle)
        front_min = self._sector_min(scan, center_angle=0.0, half_width=self.front_angle)

        if self._tick_count % self.debug_every_n == 0:
            self._dbg(
                f"[SCAN] front={front_min:.2f} m | "
                f"left={left_min:.2f} m | right={right_min:.2f} m"
            )

        # Handle all-inf case
        if math.isinf(front_min) and math.isinf(left_min) and math.isinf(right_min):
        ...
        if front_min < self.stop_distance:
            ...
            if state != self._last_state:
                self._dbg(
                    f"[DECISION] {state} | "
                    f"front={front_min:.2f} < stop={self.stop_distance:.2f}"
                )
        ...
        if front_min < self.slow_distance:
            t = (front_min - self.stop_distance) / max(1e-6, (self.slow_distance - self.stop_distance))
        ...
        eps = 1e-6
        if math.isinf(left_min): left_min = scan.range_max
        if math.isinf(right_min): right_min = scan.range_max

        diff = (right_min - left_min) / max(eps, (left_min + right_min))
